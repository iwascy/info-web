...
export default function SyncPage() {
  ...
  return (
    <Shell title="同步任务" subtitle="所有数据同步、迁移、批处理任务的实时进度。">
      ...
      {hero ? <Link href={`/sync/${hero.task_id}`} className={`card card-pad hoverable route-card mb-20 ${hero.status === "error" ? "card-error" : ""}`}><div className="row wrap"><div><div className="text-muted">当前任务</div><h2>{hero.name}</h2><div className="cell-key">{hero.service_key} · {hero.task_id}</div></div><span className="spacer" /><Badge status={hero.status} /></div><div className="progress-row mt-20"><Progress value={hero.progress} large tone={hero.status === "error" ? "red" : "blue"} /><span className="pct t-blue">{Math.round(hero.progress || 0)}%</span></div><div className="stat-grid mt-16"><div className="stat-box"><div className="k">阶段</div><div className="v">{hero.stage || "—"}</div></div><div className="stat-box"><div className="k">处理</div><div className="v">{fmtCompact(hero.processed)} / {fmtCompact(hero.total)}</div></div><div className="stat-box"><div className="k">最近更新</div><div className="v">{fmtRelative(hero.updated_at)}</div></div></div></Link> : null}
      ...
      <div className="card card-pad"><div className="table-wrap"><table className="tbl"><thead><tr><th>任务</th><th>服务</th><th>状态</th><th>阶段</th><th>进度</th><th>成功 / 失败</th><th>更新时间</th></tr></thead><tbody>{tasks.map((t) => <tr key={t.task_id} className={t.status === "error" ? "row-error" : ""}><td><Link href={`/sync/${t.task_id}`}><div className="cell-title">{t.name}</div><div className="cell-key">{t.task_id}</div></Link></td><td className="cell-key">{t.service_key}</td><td><Badge status={t.status} /></td><td>{t.stage || "—"}</td><td><div className="progress-row"><Progress value={t.progress} tone={t.status === "error" ? "red" : "blue"} /><span className="pct">{Math.round(t.progress || 0)}%</span></div></td><td className="mono">{fmtCompact(t.success)} / <span className="t-red">{fmtCompact(t.failed)}</span></td><td className="text-muted nowrap">{fmtRelative(t.updated_at)}</td></tr>)}</tbody></table>{tasks.length ? null : <div className="empty"><p>暂无同步任务</p></div>}</div></div>
    </Shell>
  );
}
