...
export default function PikpakPage() {
  ...
  return (
    <Shell title="PikPak → 115 迁移" subtitle="新加坡二号机 · 网盘文件迁移进度">
      ...
      <div className="card card-pad mb-20"><div className="card-head"><h3>最近迁移文件</h3></div><div className="table-wrap"><table className="tbl"><thead><tr><th>文件</th><th>大小</th><th>路径</th><th>下载</th><th>上传</th><th>状态</th><th>耗时</th></tr></thead><tbody>{t?.recent_files?.map((f) => <tr key={f.id} className={f.status === "error" ? "row-error" : ""}><td className="mono">{f.name}</td><td>{fmtBytes(f.size)}</td><td><span className={`path-tag ${pathClass(f.path)}`}>{pathLabel(f.path)}</span></td><td><Speed value={f.download_speed} arrow="down" /></td><td><Speed value={f.upload_speed} arrow="up" /></td><td><Badge status={f.status === "error" ? "error" : f.status} /></td><td>{f.duration || "—"}</td></tr>)}</tbody></table></div></div>
      <div className="card card-pad"><div className="card-head"><h3>异常样本</h3></div>{t?.error_samples?.map((e) => <div key={e.id} className="modal-warn"><b>{e.code}</b> · {e.file}<br />{e.reason}<pre className="codeblock json-pre mt-12">{JSON.stringify(e.payload, null, 2)}</pre></div>)}</div>
    </Shell>
  );
}
