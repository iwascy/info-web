...
export default function PikpakPage() {
  const { data: t, error, mutate } = useSWR<SyncTask>("/api/sync-tasks/pikpak_115_main", fetcher, { refreshInterval: 10000, shouldRetryOnError: false });
  if (error) {
    return (
      <Shell title="PikPak → 115 迁移" subtitle="等待真实迁移任务上报">
        <div className="card card-pad">
          <div className="empty"><h4>暂无真实迁移任务</h4><p>收到 task_id 为 pikpak_115_main 的进度上报后，这里会显示专属迁移面板。</p></div>
        </div>
      </Shell>
    );
  }
  ...
  return (
    <Shell title="PikPak → 115 迁移" subtitle="新加坡二号机 · 网盘文件迁移进度">
      <div className={`card card-pad mb-20 ${t?.status === "paused" ? "card-error" : ""}`}><div className="row wrap" style={{ gap: 16 }}><span className="cell-ico ic-blue" style={{ width: 48, height: 48 }}>{pickIcon("sync", 24)}</span><div style={{ minWidth: 240 }}><div className="row gap-12"><h2>{t?.name || "等待迁移任务"}</h2><Badge status={t?.status || "unknown"} /></div><div className="text-muted mono" style={{ fontSize: 12.5 }}>{t?.service_key || "—"} · {t?.task_id || "—"}</div></div><span className="spacer" /><div className="row gap-8"><button className={running ? "btn btn-ghost" : "btn btn-primary"} onClick={pauseResume} disabled={!t}>{running ? <Pause size={16} /> : <Play size={16} />} {running ? "暂停" : "恢复"}</button><button className="btn btn-ghost btn-icon" onClick={() => mutate()}><RefreshCw size={16} /></button></div></div><hr className="divider" /><div className="stat-grid"><div className="stat-box"><div className="k">运行状态</div><div className={`v ${running ? "t-green" : "t-yellow"}`}>{t?.status || "未知"}</div></div><div className="stat-box"><div className="k">调度时间窗</div><div className="v" style={{ fontSize: 16 }}>{t?.window_start && t.window_end ? `${t.window_start}-${t.window_end}` : "—"}</div></div><div className="stat-box"><div className="k">断点游标</div><div className="v mono t-cyan" style={{ fontSize: 16 }}>{t?.cursor || "—"}</div></div><div className="stat-box"><div className="k">最近上报</div><div className="v" style={{ fontSize: 16 }}>{fmtRelative(t?.updated_at)}</div></div></div></div>
      ...
      <div className="card card-pad"><div className="card-head"><h3>异常样本</h3></div>{t?.error_samples?.length ? t.error_samples.map((e) => <div key={e.id} className="modal-warn"><b>{e.code}</b><br />{e.reason}<pre className="codeblock json-pre mt-12">{JSON.stringify(e.payload, null, 2)}</pre></div>) : <div className="empty"><p>暂无异常样本</p></div>}</div>
    </Shell>
  );
}
