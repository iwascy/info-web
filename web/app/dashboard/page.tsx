...
const sysLabels: Record<string, { label: string; tone: string; unit: string }> = {
  mem: { label: "内存", tone: "purple", unit: "MB" },
  net: { label: "网络吞吐", tone: "cyan", unit: "MB/s" }
};
...
export default function DashboardPage() {
  ...
  const healthySeries = d?.services?.map((_, i, xs) => {
    const slice = xs.slice(0, i + 1);
    return slice.filter((s) => s.status === "healthy").length;
  });
  const errorSeries = d?.services?.map((_, i, xs) => {
    const slice = xs.slice(0, i + 1);
    return slice.filter((s) => s.status === "error").length;
  });
  const alertSeries = d?.alerts?.map((_, i) => i + 1);
  const sysEntries = Object.entries(d?.sys || {}).filter((entry): entry is [string, { value: number; series: number[] }] => Boolean(entry[1]));
  return (
    <Shell title="总览" subtitle="我的服务现在还活着吗？同步任务进行到哪了？">
      <div className="grid grid-4 mb-20">
        <MetricCard label="服务总数" value={d?.total_services ?? "—"} note={<><span className="t-green">{d?.healthy ?? 0} 正常</span> · <span className="t-blue">{d?.running ?? 0} 运行</span></>} tone="blue" icon="server" series={serviceSeries} />
        <MetricCard label="正常运行" value={d?.healthy ?? "—"} note={<>可用率 <span className="t-green tabnum">{uptime}</span></>} tone="green" icon="checkCircle" series={healthySeries} />
      ...
      <div className="cols-12 mb-20">
        ...
        <div className="span-3 card card-pad hoverable">
          <div className="card-head"><h3>运行指标</h3></div>
          {sysEntries.length ? sysEntries.map(([k, item]) => {
            const meta = sysLabels[k] || { label: k.toUpperCase(), tone: "blue", unit: "" };
            return <div className="row" style={{ justifyContent: "space-between", marginBottom: 16 }} key={k}><div className="kv"><span className="k">{meta.label}</span><span className={`v t-${meta.tone}`}>{item.value} {meta.unit}</span></div><Sparkline series={item.series} color={meta.tone === "purple" ? "#975cff" : "#17d5eb"} width={90} height={38} /></div>;
          }) : <div className="empty"><p>暂无运行指标</p></div>}
        </div>
      </div>
      ...
      <div className="cols-12">
        <div className="span-8 card card-pad hoverable">
          ...
          {d?.services?.length ? <div className="table-wrap"><table className="tbl"><thead><tr><th>服务</th><th>类型</th><th>状态</th><th>进度</th><th>最近心跳</th></tr></thead><tbody>
          ...
          </tbody></table></div> : <div className="empty"><h4>暂无服务</h4><p>接入真实服务后会显示在这里</p></div>}
        </div>
        <div className="span-4">
          <div className="card card-pad hoverable mb-20"><div className="card-head"><h3>任务进度</h3><span className="spacer" /><Link href="/sync" className="btn btn-ghost btn-sm"><ArrowRight size={14} /></Link></div>
            {d?.sync_tasks?.length ? d.sync_tasks.slice(0, 5).map((t) => <Link key={t.task_id} href={`/sync/${t.task_id}`} className="route-card mb-16"><div className="row"><span style={{ fontWeight: 600 }}>{t.name}</span><span className="spacer" /><Badge status={t.status} /></div><div className="progress-row mt-8"><Progress value={t.progress} tone={t.status === "error" ? "red" : "blue"} /><span className="pct t-blue">{Math.round(t.progress || 0)}%</span></div><div className="text-muted" style={{ fontSize: 12 }}>{fmtCompact(t.processed)} / {fmtCompact(t.total)}</div></Link>) : <div className="empty"><p>暂无同步任务</p></div>}
          </div>
