...
import { createDaemonChannel, } from "@getpaseo/relay/e2ee";
import { buildRelayWebSocketUrl } from "@getpaseo/protocol/daemon-endpoints";
const CONTROL_PING_INTERVAL_MS = 10000;
const CONTROL_STALE_TIMEOUT_MS = 30000;
const CONTROL_READY_TIMEOUT_MS = 8000;
const RELAY_WEBSOCKET_OPTIONS = { handshakeTimeout: 10000, perMessageDeflate: false };
function createDefaultRelayWebSocket(url) {
    return new WebSocket(url, RELAY_WEBSOCKET_OPTIONS);
}
function normalizeRelaySendPayload(data) {
...
function tryParseControlMessage(raw) {
    try {
        let text;
        ...
        if (parsed.type === "sync" && Array.isArray(parsed.connectionIds)) {
            const connectionIds = parsed.connectionIds.filter((id) => typeof id === "string" && id.trim().length > 0);
...
export function startRelayTransport({ logger, attachSocket, relayEndpoint, relayUseTls, serverId, daemonKeyPair, createWebSocket = createDefaultRelayWebSocket, }) {
    const relayLogger = logger.child({ module: "relay-transport" });
    let stopped = false;
    let controlWs = null;
    let reconnectTimeout = null;
    let reconnectAttempt = 0;
    const dataSockets = new Map(); // connectionId -> ws
    let controlKeepaliveInterval = null;
    let controlReadyTimeout = null;
    ...
    const connectControl = () => {
        ...
        socket.on("open", () => {
            ...
            controlReadyTimeout = setTimeout(() => {
                ...
                if (controlConnected)
                    return;
                relayLogger.warn({ url, connectionId, waitedMs: CONTROL_READY_TIMEOUT_MS }, "relay_control_ready_timeout_terminating");
                try {
                    socket.terminate();
                }
                catch {
            ...
            controlKeepaliveInterval = setInterval(() => {
                ...
                if (staleForMs > CONTROL_STALE_TIMEOUT_MS) {
                    relayLogger.warn({ url, staleForMs, connectionId, staleTimeoutMs: CONTROL_STALE_TIMEOUT_MS }, "relay_control_stale_terminating");
                    try {
                        socket.terminate();
                    }
                    catch {
        ...
        socket.on("close", (code, reason) => {
            if (controlWs !== socket)
                return;
            relayLogger.warn({ code, reason: reason?.toString?.(), url, connectionId }, "relay_control_disconnected");
            controlWs = null;
            if (controlKeepaliveInterval) {
                clearInterval(controlKeepaliveInterval);
                controlKeepaliveInterval = null;
    ...
    const ensureClientDataSocket = (connectionId) => {
        if (stopped)
            return;
        if (!connectionId)
            return;
        if (dataSockets.has(connectionId))
            return;
        const url = buildRelayWebSocketUrl({
            endpoint: relayEndpoint,
            useTls: relayUseTls,
        ...
        socket.on("close", (code, reason) => {
            clearTimeout(openTimeout);
            relayLogger.warn({ code, reason: reason?.toString?.(), url, connectionId }, "relay_data_disconnected");
...
async function attachEncryptedSocket(socket, daemonKeyPair, logger, attachSocket, metadata) {
    try {
...
function createRelayTransportAdapter(socket, logger) {
...
function createEncryptedSocket(channel, emitter) {
    let readyState = 1;
...
function normalizeMessageData(data, isBinary) {
    if (!isBinary) {
        if (typeof data === "string")
            return data;
